describe('Multi-select', () => {
  ...
  it('should show summary text when too many selections', () => {
    cy.get('ay-multi-select').first().within(() => {
      cy.get('.ay-multi-select-box').click()
      cy.get('.ay-multi-select-toolbar-btn--text').first().click() // Tout sélectionner
      cy.get('.ay-multi-select-summary').should('be.visible')
        .and('contain.text', 'valeurs sélectionnées')
    })
  })
  ...
  it('should deselect all via toolbar', () => {
    cy.get('ay-multi-select').first().within(() => {
      cy.get('.ay-multi-select-box').click()
      cy.get('.ay-multi-select-toolbar-btn--text').first().click() // Tout sélectionner
      cy.get('.ay-multi-select-toolbar-btn--text').last().click() // Tout désélectionner
      cy.get('.ay-chip').should('have.length', 0)
      cy.get('.ay-multi-select-summary').should('not.exist')
    })
  })
